...
namespace ORB_SLAM3
{
    ...
    public:
        ...
        TwoViewReconstruction(const Eigen::Matrix3f& k, const cv::Mat& lineMatch, float sigma = 1.0, int iterations = 200);
        ...
        bool ReconstructWithLine(const Frame &F1, const Frame &F2, const std::vector<int> &vMatches12, const std::vector<int> &vMatchLines12,
                         Sophus::SE3f &T21, std::vector<cv::Point3f> &vP3D, std::vector<Vector6f> &vLinePluker, std::vector<Vector6f> &vLine3dW,
                         std::vector<bool> &vbTriangulated, std::vector<bool> &vbLineTriangulated);

        cv::Mat mLineMatchImg;
    ...
    private:
        ...
        bool ReconstructPlukerLine(std::vector<Match>& vMatchLines12, Eigen::Matrix3f &K, Sophus::SE3f &T21, std::vector<Vector6f> &vLinePluker,
                                   std::vector<ORB_SLAM3::Vector6f> &vLine3dW, std::vector<bool> &vbLineTriangulated, int minTriangulated);
        ...
        Eigen::Vector3d UnProjectEig(const Eigen::Vector3d &p2D);

        Eigen::Matrix3d GetPlukerKMatrix();
        ...
        std::vector<line_descriptor::KeyLine> mvKeyLines1;
        std::vector<line_descriptor::KeyLine> mvKeyLines2;

        // Current Matches from Reference to Current Line Part
        std::vector<Match> mvMatchLines12;
        std::vector<bool> mvbMatchedLine1;
