...
public:
    ...
    // Constructor for Monocular cameras with line.
    Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor, LineExtractor* lineExtractor, ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    void LineDescriptorMAD( vector<vector<DMatch>> lineMatches, double &nn_mad, double &nn12_mad) const;
    ...
    void SetManhWorldPose(const Sophus::SE3<float> &Tcm);
    ...
    Vector6f Obtain3DLine(const int &i);
    ...
    inline Sophus::SE3<float> GetManhWorldPose() const {
        //TODO: can the Frame pose be accsessed from several threads? should this be protected somehow?
        return mTcm;
    }
...
private:
    ...
    Eigen::Matrix<float,3,3> mRwc = Eigen::Matrix3f::Identity();
    Eigen::Matrix<float,3,1> mOw;
    Eigen::Matrix<float,3,3> mRcw = Eigen::Matrix3f::Identity();;
    Eigen::Matrix<float,3,1> mtcw;
    bool mbHasPose;

    Sophus::SE3<float> mTcm;
...
public:
    ...
    std::vector<Vector3d> mvKeyLineFunctions;

    std::vector<Vector3d> mvLineVps;
    std::vector<int> mvLineToVpIds;
    std::vector<vector<int>> mvLineGroupClusters;
    std::vector<Eigen::Vector3d> mvLineVpsAxis;
    std::vector<Eigen::Vector3d> mvProjLineVpsAxis;

    bool mbDetectManhFrame;

    std::vector<Vector3d> mvInitialManhAxis;
    Eigen::Matrix3d mRcm;

    // Corresponding stereo coordinate and depth for each keypoint.
    std::vector<MapPoint*> mvpMapPoints;
    std::vector<MapLine*> mvpMapLines; // keylines association
    ...
    cv::Mat mLineDescriptors, mLineDescriptorsFloat;

    std::vector<bool> mvKeyLinesIsTriangulation;
    ...
    // line extractor member
    vector<cv::Point3d> mv3DLineforMap;
    vector<Vector6f > mvLines3dPoints;
    std::vector<Vector6f> mvLinesPluker;

//    std::vector<VanishingDirection> vVanishingDirection;
    std::vector<vector<cv::Point2d>> vVanishingLinex;
    std::vector<vector<cv::Point2d>> vVanishingLiney;
    std::vector<vector<cv::Point2d>> vVanishingLinez;
