...
namespace ORB_SLAM3 {
    ...
    public:
        ...
        bool ReconstructWithTwoViewsByPL(const Frame &F1, const Frame &F2, const std::vector<int> &vMatches12, const std::vector<int>& vMatchLines12, Sophus::SE3f &T21,
                                         std::vector<cv::Point3f> &vP3D, std::vector<Vector6f>& vLinePluker, std::vector<Vector6f>& vL3D, std::vector<bool> &vbTriangulated, std::vector<bool>& vbLineTriangulated);

        cv::Mat toK();
        Eigen::Matrix3f toK_();

        cv::Mat toPlukerK();
        Eigen::Matrix3f toPlukerK_();
